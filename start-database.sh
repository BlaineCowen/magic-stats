...
start_db() {
    echo "Starting NFL Stats database..."
    docker-compose up -d
...
stop_db() {
    echo "Stopping NFL Stats database..."
    docker-compose down
}
...
backup_db() {
    ...
    BACKUP_FILE="nfl_stats_backup_$(date +%Y%m%d_%H%M%S).sql"
    docker-compose exec -T db pg_dump -U nfl_user nfl_stats > "./backups/$BACKUP_FILE"
