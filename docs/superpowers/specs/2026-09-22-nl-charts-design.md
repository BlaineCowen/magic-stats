...
- **Chart gate, two tiers (final review, F1):** `asksForChart` splits into `EXPLICIT_CHART` (plot/graph/visualize/scatter/trend/chart — but not "chart" right after "depth", so "depth chart" doesn't trigger — /over time) and `VERSUS` (vs/versus alone). An explicit word behaves as before (model pick, else rule-inferred, of any type). `vs`/`versus` alone only infers a scatter, and only when the question actually names both axis columns (`mention` finds each one) — so "How did Mahomes do vs the Ravens?" stays a table instead of a bogus scatter. Neither present: no chart, regardless of the model's pick.
- **"Chart this" prefers bars on a sorted leaderboard (final review, F10):** `inferSpec`'s no-intent shape order is now sorted time column → line; else a sorted metric column alongside a text column (already-ranked query results, the common "Chart this" case) → bar; else ≥2 metric columns → scatter; else → bar. Previously two metrics always won the scatter slot even when the rows were already an ORDER BY leaderboard.
